...
const newContactInfo = graduates.map(function(student){
...
const result1 = graduates.filter(function(school){
  if(~school.university.indexOf("Uni")){
  unisWithUni.push(school.univeristy);
}
});
...
array with only the animal_name and scientific_name of each animal. displayNames will be an array of strings,
...
zooAnimals.forEach(function(animal){
  ...
  displayNames.push(animal.scientific_name);
})
...
const map1 = zooAnimals.map(animal => animal.animal_name.toLowerCase());
...
array of objects called lowPopulationAnimals which contains only the animals with a population less
than 5.

*/
const lowPopulationAnimals = [];

const result = zooAnimals.filter(function(animal){
...
from all the zoos using the .reduce() method. Remember the reduce method takes
two arguments: a callback (which itself takes two args), and an initial value for the count.

*/
// const populationTotal = 0

const totalPopulation = zooAnimals.reduce(function(accumulator, animal){
  return accumulator + animal.population;
}, 0);
console.log(`Our zoos total population across the USA is: ${totalPopulation}`);
